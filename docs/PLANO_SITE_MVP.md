...
Um site em português, inicialmente sem login, no qual a pessoa:

1. envia uma planilha `.xlsx` ou `.csv`;
2. vê quais dados e qual formato foram reconhecidos e confirma o modelo sugerido;
3. informa título, autores, fonte e, quando existir nos dados, o período;
4. vê a prévia e baixa o PDF. Para a figura das Regionais, também pode baixar PNG.
...
- **Dois serviços no mesmo repositório:** interface Next.js publicada na Vercel, com raiz em `frontend/`, e API FastAPI publicada no Render por Docker. O Next.js encaminha `/api/*` ao Render; upload e download continuam na mesma origem para o navegador. A URL do Render é configurada em `API_BACKEND_URL` no projeto Vercel.
...
- **Sem login nem banco de dados nesta versão:** qualquer pessoa com a URL pode usar o gerador. Não há histórico nem registro confiável de quem gerou cada relatório; o campo "autores" é informado pela pessoa e não comprova identidade.
...
3. **Montar a interface curta:** upload, confirmação dos dados, escolhas compatíveis, prévia e download. Campos avançados ficam recolhidos. Estados de carregamento/erro precisam estar claros.
...
- **`v0.1.0` — piloto web:** fluxo completo com os dois formatos definidos acima, sem login, com prévia e download implementados localmente. Falta validar a URL publicada. A versão aparece no site e na metodologia do PDF.
...
- **Versão posterior:** contas individuais e um banco para guardar relatórios, data de geração e autoria verificada. Definir acesso, retenção e exclusão antes de armazenar planilhas ou resultados.
