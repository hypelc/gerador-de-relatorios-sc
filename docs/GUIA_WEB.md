...
3. A usuária confere os dados e escolhe um modelo visual compatível. Séries anuais oferecem painéis, linhas e barras; o mapa aceita um subconjunto dos oito códigos reais de macrorregiões de SC. Cobertura vacinal usa faixas fixas; outros indicadores usam escala contínua comum aos anos selecionados e exibem a unidade informada. Regiões ausentes da tabela ou sem valor no ano aparecem cinza e hachuradas; um ano totalmente sem valores não gera mapa. Pizza só é oferecida para contagens regionais aditivas, de um ano, sem lacunas; taxas e coberturas não são partes de um total. Tabelas regionais de período único geram barras com o valor estadual como referência, quando houver.
...
Os números vêm da planilha. Anos fora de ordem são apresentados cronologicamente mantendo a correspondência original entre ano e valor. Colunas `MÉDIA` e `TOTAL` não entram na série anual. Células vazias continuam ausentes, zeros continuam zero e valores acima de 100% são preservados. Se o indicador não estiver identificado, o sistema usa `Valor informado na planilha`, sem supor que seja cobertura vacinal. As regiões fictícias dos exemplos não são desenhadas no mapa de SC. O mapa das 17 Regionais aceita apenas alguns nomes, desde que correspondam exatamente à composição geográfica utilizada e exista ao menos uma Regional com valor numérico. Nomes não reconhecidos são rejeitados; Regionais ausentes ou com célula vazia aparecem como `Sem dado`. A linha de Santa Catarina é opcional e, quando ausente, seu valor também aparece como `Sem dado`. O sistema não estima os valores faltantes. Quando o arquivo não atende a um contrato, o site mostra o problema em vez de escolher um relatório arbitrário.

Para testar o mapa parcial sem dados reais, importe `tests/fixtures/vacina_sc_parcial_FICTICIA.csv`. O ano 2021 não contém valores e fica indisponível para o mapa; em 2022, seis regiões aparecem como `Sem dado`, uma tem valor zero e outra tem valor acima de 100%. Este arquivo contém apenas números fictícios de teste.

Para testar as outras escalas e o contrato regional parcial, use `tests/fixtures/taxa_sc_macrorregioes_FICTICIA.csv` (taxa anual com dois códigos reais) e `tests/fixtures/regionais_sc_parcial_FICTICIA.csv` (duas Regionais com valor, uma célula vazia e SC sem valor). Os números também são fictícios.
